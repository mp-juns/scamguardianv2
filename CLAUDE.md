...
```bash
./scripts/start_stack.sh    # uvicorn + next dev를 nohup으로 실행
...
| `kakao_formatter.py` | ScamReport → 카카오 챗봇 응답 JSON | `format_response(report_dict)` |
...
카카오 오픈빌더 Skill 엔드포인트. 입력 유형별 처리 방식:

| 입력 | 처리 |
|------|------|
| 텍스트 발화 | 즉시 동기 분석 (STT 없음, 5초 이내) |
| YouTube/영상 URL (utterance) | callbackUrl 있으면 백그라운드 처리 → 60초 내 결과 POST |
| 카카오 파일 업로드 (`action.params.file` / `.video`) | URL 추출 후 callback 처리 |
| callbackUrl 없는 URL | 빠른 동기 처리 (timeout 위험 — 오픈빌더 callback 활성화 권장) |

**소스 추출 우선순위**: `action.params` 파일/영상 URL > utterance 내 URL 감지 > utterance 텍스트
...
- 스킬 서버 외부 노출: Tailscale Funnel 사용 (`tailscale funnel --bg 8000`)
  - 공개 URL 예시: `https://mp-windows.tail7e5dfc.ts.net/webhook/kakao`

callback 처리 시 `use_llm=True`로 Claude 보조 판정까지 포함한 고품질 분석 수행.
