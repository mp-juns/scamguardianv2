...
**문서 버전**: v1.4 (2026-05-05) — 자문·심사용 친절판 (정중체)
...
| Phase 0.6 — APK 동적 Lv 3 *(인터페이스만)* | `pipeline/apk_analyzer.analyze_apk_dynamic` | `apk_runtime_*` 5종 — 격리 가상머신에서만 동작하며, 로컬 실행은 어떤 경우에도 차단됩니다 |
...
📖 **Kim et al. (2022) HearMeOut** — SMS 가로채기 / 화면 오버레이 / 자격증명 탈취 동작을 관찰했습니다. DOI: [10.1145/3498361.3538939](https://doi.org/10.1145/3498361.3538939)
📖 **Fratantonio et al. (2017) Cloak and Dagger** — UI 가로채기 동작을 학술적으로 검증했습니다. DOI: [10.1109/SP.2017.39](https://doi.org/10.1109/SP.2017.39)
...
**5 종 신호**:
- `apk_runtime_c2_network_call` — 격리 환경에서 *실제로* C&C 도메인 호출이 관찰됩니다.
- `apk_runtime_sms_intercepted` — *가상* SMS 시뮬레이션 시 자동 가로채기·재전송 동작이 관찰됩니다.
- `apk_runtime_overlay_attack` — 가상 은행 앱 위에 가짜 화면을 *실제로* 띄웁니다.
- `apk_runtime_credential_exfiltration` — 자격증명·민감정보의 외부 송신이 관찰됩니다 (Frida 후크).
- `apk_runtime_persistence_install` — 재부팅 시 자동 시작 / DeviceAdmin 활성화가 관찰됩니다.
...
- 검출 신호화: `pipeline/signal_detector.py` 가 *status=COMPLETED* 일 때만 처리합니다.
...
3. **거짓 양성 (false positive) 정직 표시**: 위 §0.6 / §8 표를 참조하시기 바랍니다. 단독 사용 비권장 신호 4종이 명시되어 있습니다.
